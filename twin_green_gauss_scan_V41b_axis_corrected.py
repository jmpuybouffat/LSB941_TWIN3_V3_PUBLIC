...
# Green + Gaussian scan with Tumelo / focal-law axis correction
#
# Purpose:
#   Correct coordinate mapping between Tumelo EDM frame and
#   focal-law / PA frame before validating Green+Gauss EDM response.
#
# Diagnostic result:
#   Focal-law range:
#       focus_x ~ -124 to +125
...
#
# Therefore:
#       model_x = Tumelo edm_y
#       model_y = Tumelo edm_x
#
# Outputs:
...
def read_edm_tumelo_axis_corrected(path: Path) -> pd.DataFrame:
    rows = []
    ...
    with open(path, "r", encoding="utf-8-sig") as f:
        for line in f:
            ...
            try:
                ...
                edm_y = float(parts[1])
                edm_z = float(parts[2])
                rows.append((edm_x, edm_y, edm_z))
            except ValueError:
                continue

    if not rows:
        raise ValueError("No valid Tumelo EDM rows found.")

    df = pd.DataFrame(rows, columns=["edm_x_tumelo", "edm_y_tumelo", "edm_z"])
    ...
    # Axis correction:
    # Tumelo x corresponds to focal-law y
    # Tumelo y corresponds to focal-law x
    df["edm_model_x"] = df["edm_y_tumelo"]
    df["edm_model_y"] = df["edm_x_tumelo"]
    df["edm_model_z"] = df["edm_z"]
...
def compute_green_gauss_scan(hw: pd.DataFrame, X: np.ndarray, Y: np.ndarray) -> tuple[np.ndarray, pd.DataFrame]:
    ...
    for scan_id in scan_ids:
        ...
        rows.append({
            ...
            "physics_level": "V4.1b Green + Gaussian with Tumelo/focal-law axis correction",
        })
...
def create_edm_spots_axis_corrected(
    ...
    for _, edm in edm_df.iterrows():
        ...
        rows.append({
            "edm_id": edm["edm_id"],
            "side": edm["side"],
            "edm_x_tumelo": float(edm["edm_x_tumelo"]),
            "edm_y_tumelo": float(edm["edm_y_tumelo"]),
            "edm_z": float(edm["edm_z"]),
            "edm_model_x": mx,
            "edm_model_y": my,
            ...
            "status": status,
            "axis_correction": "model_x = Tumelo_y ; model_y = Tumelo_x",
            "physics_level": "V4.1b Green+Gauss sampled scan with corrected axes",
        })
...
def image_to_dataframe(X: np.ndarray, Y: np.ndarray, image: np.ndarray, label: str) -> pd.DataFrame:
    return pd.DataFrame({
        ...
        "spot_sigma_mm": SPOT_SIGMA_MM,
        "axis_correction": "model_x = Tumelo_y ; model_y = Tumelo_x",
        "physics_level": "V4.1b Green + Gaussian + corrected axis sampled scan",
    })
...
def plot_axis_corrected_scan(X: np.ndarray, Y: np.ndarray, scan_image: np.ndarray, edm_df: pd.DataFrame) -> Path:
    ...
    fig.suptitle("BYTE NDT - LSB941 V4.1b Axis-Corrected Green+Gauss Scan", fontsize=16, fontweight="bold")
    ...
    ax.set_title("Corrected frame: X = Tumelo Y, Y = Tumelo X")
    ax.set_xlabel("Model X = Tumelo Y (mm)")
    ax.set_ylabel("Model Y = Tumelo X (mm)")
    ax.grid(True)
...
def plot_axis_corrected_edm_response(
    ...
    fig = plt.figure(figsize=(14, 10))
    fig.suptitle("BYTE NDT - LSB941 V4.1b Axis-Corrected EDM Response", fontsize=16, fontweight="bold")
    ...
    ax.set_title("Localized EDM spots after Tumelo/focal-law axis correction")
    ax.set_xlabel("Model X = Tumelo Y (mm)")
    ax.set_ylabel("Model Y = Tumelo X (mm)")
    ax.grid(True)
...
def main() -> None:
    ...
    edm_df = read_edm_tumelo_axis_corrected(EDM_FILE)
    print(edm_df[["edm_id", "edm_x_tumelo", "edm_y_tumelo", "edm_model_x", "edm_model_y", "edm_z", "side"]])
    ...
    print("Plotting corrected images")
